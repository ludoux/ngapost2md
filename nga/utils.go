...
func ToSaveFilename(in string) string {
	// 参考: https://stackoverflow.com/questions/1976007/what-characters-are-forbidden-in-windows-and-linux-directory-names
...
func FindFolderNameByTid(tid int, authorId int) (string, error) {
	...
	fullPath := filepath.Join(CFGFILE_OUTPUT_PATH, folderName)
	fi, err := os.Stat(fullPath)
	if err == nil && fi.IsDir() {
		return folderName, nil
	} else if err != nil && !os.IsNotExist(err) {
		return "", fmt.Errorf("检查文件夹 %s 失败: %v", fullPath, err)
	}
	// 即不存在直接以 tid 命名的文件夹，接下来判断是否存在以 tid- 开头的文件夹
	if authorId > 0 {
		folderName = filepath.Join(CFGFILE_OUTPUT_PATH, fmt.Sprintf("%d(%d)-*", tid, authorId))
	} else {
		folderName = filepath.Join(CFGFILE_OUTPUT_PATH, fmt.Sprintf("%d-*", tid))
	}
