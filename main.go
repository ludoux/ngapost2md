...
func main() {
	...
	//默认线程数为2,仅支持1~3
	nga.CFGFILE_THREAD_COUNT = int(math.Max(1, math.Min(float64(cfg.Network.Thread), 3)))
	nga.CFGFILE_PAGE_DOWNLOAD_LIMIT = int(math.Max(-1, math.Min(float64(cfg.Network.Page_limit), 100)))
	nga.CFGFILE_GET_IP_LOCATION = cfg.Post.Ip
	nga.CFGFILE_ENHANCE_ORI_REPLY = cfg.Post.Reply
	nga.CFGFILE_USE_LOCAL_SMILE_PIC = cfg.Post.Local_smile
	nga.CFGFILE_LOCAL_SMILE_PIC_PATH = cfg.Post.Local_smile_path
	nga.CFGFILE_USE_TITLE_AS_FOLDER_NAME = cfg.Post.Title_dir_name
	nga.CFGFILE_USE_TITLE_AS_MD_FILE_NAME = cfg.Post.Title_md_name
